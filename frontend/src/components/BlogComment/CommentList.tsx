...
import { CommonCommentType } from '@/lib/types'
import NoData from '@/components/NoData'
import { timeAgo } from '@/utils/date'

const CommentList = ({ blog_comments }: { blog_comments: CommonCommentType[] }) => {
  if (!blog_comments || blog_comments.length < 1) {
  ...
  return (
    <div className="space-y-4 py-10 px-5">
      {blog_comments.map((comment) => (
        <div key={String(comment.id)} className="flex space-x-4">
          {/* Comment User Icon */}
          ...
          <div className="flex flex-col space-y-1.5">
            ...
            <p className="text-base font-normal text-gray-800 dark:text-gray-400 leading-snug">{comment.comment}</p>
            <span className="text-sm text-gray-500 leading-tight">{timeAgo(comment.created_at)}</span>
          </div>
        </div>
      ))}
    </div>
