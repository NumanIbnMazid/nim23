            ...
            <Link href={`/blogs/${blog.slug}`} title="View Blog Details" shallow={true}>
                ...
                >
                    ...
                    <div className="flex flex-col w-full h-full px-2 pb-2 mt-2 sm:mt-0 sm:p-1 lg:py-5 md:pr-5">
                        <h2 className="font-bold text-neutral-900 md:text-xl dark:text-neutral-200">
                            {blog.title}
                        </h2>
                            ...
                            <div className="z-10 flex items-center gap-3 font-barlow">
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-xs">
                                        {getFormattedDate(new Date(blog.created_at))}
                                    </span>
                                </div>
                            </div>

                            {/* ✅ Fix: Always display reading time, and use `hasCode` */}
                            {readingTime && !hasCode && (
                                <p className="flex items-center justify-between text-xs font-medium text-gray-500 dark:text-dark-3 md:text-sm">
                                    <span className="px-2 py-1 text-xs rounded bg-gray-700 text-gray-50">
                                        {readingTime.text}
                                    </span>
                                </p>
                            )}
