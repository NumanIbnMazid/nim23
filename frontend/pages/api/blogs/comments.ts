...
import { BLOG_COMMENT_ENDPOINT, BACKEND_API_TOKEN } from '@lib/backendAPI'
...
export default async function handler(
  ...
  try {
    ...
    const response = await fetch(
      BLOG_COMMENT_ENDPOINT + `?slug=${slug}`,
      {
        method: method,
        headers: {
    ...
    if (response.ok) {
      ...
      const messageText = method == "POST" ? "Error posting blog comment" : "Error fetching blog comments"
